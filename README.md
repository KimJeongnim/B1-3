...
두 도구 모두 동일한 기능을 수행하지만 워크플로우 구성 방식과 사용성에서 차이를 확인할 수 있었다.
...
| Discord 연동 | Webhook 또는 API 사용 | 기 모듈 제공 |
...
기존에는 Google Sheets의 출석 정보를 확인한 후 Discord에 직접 공지를 작성해야 했지만, 자동화를 통해 새로운 행이 등록되는 즉시 알림이 자동으로 전송되도록 구현하였다.자동화를 통해 새로운 행이 등록되는 즉시 알림이 전송되도록 구현하였다.

---

## 2. 자동화 업무 정의

학생의 입실 및 퇴실 정보를 Discord 채널에 자동으로 안내하는 반복 업무를 자동화하였다.

### 자동 실행 구조

Google Sheets에 새로운 행이 등록되면 Google Sheets Trigger가 자동으로 실행된다.

이후 IF 조건 분기를 통해 입실과 퇴실을 구분한 뒤 Discord 채널에 자동으로 알림을 전송하도록 구현하였다.
...
앞으로도 반복되는 업무에 자동화를 적용하여 업무 효율성을 높이는 다양한 프로젝트를 수행해 보고 싶다.
