...
동일한 자동화 구조를 **n8n**과 **Make**에서 각각 구현하여 기능과 사용성을 비교하였다.
...
| Discord 연동 | Webhook 또는 API 사용 | 기몬 모듈 제공 |
...
기존에는 출석 정보를 확인한 후 관리자가 직접 Discord에 공지를 작성해야 했지만, 자동화를 통해 새로운 행이 등록되는 즉시 알림이 전송되도록 구현하였다.
...
Google Sheets에 새로운 행이 추가되는 순간 Trigger가 자동으로 실행되며 사용자의 별도 조작 없이 IF 조건을 거쳐 Discord 알림이 자동으로 전송된다. 따라서 반복적으로 Discord 공지를 작성하는 업무를 자동화하였다.
...
제출 화면에 포함될 수 있는 API Key, Webhook URL, Discord Token 및 Google 계정 정보는 모두 마스킹(***) 처리하여 민감한 정보가 노출되지 않도록 하였다.
