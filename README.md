...
| 비교 항목 | n8n | Make |
|-----------|------|------|
| UI/UX | 노드 기반 인터페이스 | 시각적인 인터페이스 |
| 설정 난이도 | 중간 | 쉬움 |
| Google Sheets 연동 | 지원 | 지원 |
| Discord 연동 | 지원 | 지원 |
| 조건 분기 | IF Node | Router |
| 무료 플랜 | 비교적 넉넉 | Operation 제한 |
| 실행 로그 | Executions | Scenario History |
| 학습 난이도 | 다소 높음 | 비교적 쉬움 |
...
학생의 출석 정보를 Google Sheets에 입력하면 Discord 채널에 자동으로 출석 알림을 전송하는 반복 업무를 자동화하였다.
...
- 무료 플랜에서도 다양한 기능을 제공한다.
- Google Sheets와 Discord 연동이 가능하다.
- IF 조건 분기를 쉽게 구현할 수 있다.
- 실행 로그를 확인하기 편리하여 테스트와 디버깅이 용이하다.
